...
// Making this view agnostic to the models so can be used in other apps,
// to save time is using the same view model as the InsightView but should be using a different one.
struct RingView: View {
    
    @ObservedObject var viewModel: InsightsViewModel
    
    private func ratio(for categoryIndex: Int) -> Double {
        Double(viewModel.categoriesInsightsViewModel[categoryIndex].ratio)
    }
    
    private func offset(for categoryIndex: Int) -> Double {
        Double(viewModel.categoriesInsightsViewModel[categoryIndex].offset)
    }
    ...
    private func percentageText(for categoryIndex: Int) -> String {
        let value = (ratio(for: categoryIndex) * 100).formatted(hasDecimals: false)
        return value != "0" ? value + "%" : ""
    }
    
    var body: some View {
        ZStack {
            ForEach(viewModel.categoriesInsightsViewModel.indices) { categoryIndex in
                PartialCircleShape(
                    offset: offset(for: categoryIndex),
                    ratio: ratio(for: categoryIndex)
                )
